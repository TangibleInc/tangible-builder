...
export class JsonSchemaGenerator {
    ...
    private parseCommentsIntoDefinition(symbol: ts.Symbol, definition: Definition, otherAnnotations: {}): void {
        ...
        jsdocs.forEach(doc => {
            ...
            if (validationKeywords[name] || this.userValidationKeywords[name]) {
                ...
                if (!otherAnnotations.meta) otherAnnotations.meta = {}
                if (typeof otherAnnotations.meta[name]!=='undefined') {
                  if (!Array.isArray(otherAnnotations.meta[name])) {
                    otherAnnotations.meta[name] = [ otherAnnotations.meta[name] ]
                  }
                  otherAnnotations.meta[name].push(text)
                } else {
                  otherAnnotations.meta[name] = text
                }
            }
        });
    }
    ...
    private getTypeDefinition(typ: ts.Type, asRef = this.args.ref, unionModifier: string = "anyOf", prop?: ts.Symbol, reffedType?: ts.Symbol, pairedSymbol?: ts.Symbol): Definition {
        ...
        if (otherAnnotations.meta) {
          Object.assign(returnedDefinition, otherAnnotations.meta)
        }
