/**
 * Based on: https://raw.githubusercontent.com/ngryman/gulp-bro
 */

'use strict'

const browserify = require('browserify')
// const incremental = require('browserify-incremental') // CHANGED: No longer using incremental build
...
function createBundler(opts, file, transform) {
  ...
  if (bundler) {
    ...
    bundler = browserify(opts) // CHANGED: Object.assign(opts, incremental.args)
    // only available via method call (#25)
    if (opts.external) {
      bundler.external(opts.external)
    }
    // incremental(bundler) // CHANGED
